...
%
% If the inputs coming to this functions are string convert them into
% numbers
...
%% Checking if the number of gaussians are in the power of 2 or not.
...
%% Checking if the data array is passed in string format 
...
% dataList is nothing but PF(:) i.e each column stacked one below the other
% NOTE: PF = 2x50 cell, PF(:) returns 100x1 cell struture
nfiles = length(dataList);  % length of dataList is 100

%% Finding GMM hyperparameters
fprintf('\n\nInitializing the GMM hyperparameters ...\n');
[gm, gv] = comp_gm_gv(dataList); % Compute and mean and variance of the data
gmm = gmm_init(gm, gv);   % initialize mu, sigma and weights of gmm in a structure 
...
if ~iscellstr(datalist)
    fid = fopen(datalist, 'rt');  % opens file in text mode with permissions 'r' for reading and 't' for text mode
    filenames = textscan(fid, '%s');  % read formatted data in form of string
    fclose(fid);
